    ...
    internal static class Program
    {
        ...
        private static int Main()
        {
            ...
            try
            {
                ...
                Assert(Math.Abs(firstMetadata.DepthPt - autoSvg.DepthPt * 1.1) < 0.01,
                    "The 10 pt StemTeX inline box was not scaled to Word's 11 pt insertion font.");
                var compensatedSvg = Encoding.UTF8.GetString(LaTeXBlockService.ApplyFractionalBaselineCompensation(
                    autoSvg.Bytes, autoSvg.DepthPt, 1.1));
                ...
                Assert(inserted.Range.Font.Position == -(int)Math.Round(firstMetadata.DepthPt + 0.75, MidpointRounding.AwayFromZero),
                    "Word baseline compensation does not include the inline SVG effect extent.");
                ...
                Assert(reopened.Range.Font.Position == -(int)Math.Round(reopenedMetadata.DepthPt + 0.75, MidpointRounding.AwayFromZero),
                    "Baseline compensation did not survive save and reopen.");

                var updated = service.UpdateBlock(reopened, updatedSource, 420, LaTeXBlockLayoutMode.Auto, profile);
